...
function CanvasGrid() {
    ...
    const activeScreen = screenSizeData.activeScreens[screenSizeData.selectedScreen];
    let columnCells = 0;
    let columnValue = "";
    let rowCells = 0;
    let rowValue = "";
    ...
    return <div id="canvas-grid">
        <div id="grid-columns" style={columnStyles}>
            {[...Array(columnCells)].map((_, index) => <div key={index}></div>)}
        </div>
        <div id="grid-rows" style={rowStyles}>
            {[...Array(rowCells)].map((_, index) => <div key={index}></div>)}
        </div>
    </div>;
}
